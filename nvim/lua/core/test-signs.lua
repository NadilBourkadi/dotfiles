...
-- Strip ANSI escape codes from a string (handles 256-color, truecolor, and OSC sequences)
local function strip_ansi(str)
  return str:gsub("\027%[%d*[;%d]*[A-Za-z]", ""):gsub("\027%][^\a]*\a", "")
end
...
function M.parse_pytest_output(output_lines)
  ...
  local statuses = { "PASSED", "FAILED", "SKIPPED", "ERROR" }
...
-- Called by TermClose autocmd when test terminal exits
function M.on_complete(term_buf)
  if not term_buf or not vim.api.nvim_buf_is_valid(term_buf) then
    return
  end
  if not M.state.test_buf or not vim.api.nvim_buf_is_valid(M.state.test_buf) then
  ...
  local output_lines = get_terminal_output(term_buf)
  local test_results = M.parse_pytest_output(output_lines)
  local test_functions = M.find_test_functions(M.state.test_buf)
...
function M.close_test_terminal()
  local test_patterns = { "test", "jest", "pytest", "go test", "cargo test" }
  local bufs_to_delete = {}

  -- Single pass: collect terminal buffers matching test patterns, close their windows
  for _, buf in ipairs(vim.api.nvim_list_bufs()) do
...
function M.run_test(cmd)
  ...
  -- Find terminal buffer after command executes
