...
-- Using vim-test with custom gutter indicators
...
return {
  ...
  config = function()
    ...
    -- ============================================
    -- Gutter indicators for test results
    -- ============================================

    -- Ensure signcolumn is visible
    vim.opt.signcolumn = "yes"

    -- Define signs for test results (using simple text for compatibility)
    ...
    local function parse_pytest_output(output_lines)
      local results = {}
      ...
      for _, line in ipairs(output_lines) do
        ...
        if clean_line:match("::") and clean_line:match("PASSED") then
          -- Extract the last ::word before PASSED
    ...
    local function place_test_signs(bufnr, test_functions, test_results)
      clear_test_signs(bufnr)
      ...
      for test_name, line_num in pairs(test_functions) do
        ...
        if sign_name then
          ...
          if sign_id > 0 then
            table.insert(test_state.signs_placed, { id = sign_id, bufnr = bufnr })
          end
        end
      end
    end
    ...
    local function place_running_signs(bufnr)
      clear_test_signs(bufnr)
      local test_functions = find_test_functions(bufnr)
    ...
    local function update_signs_from_terminal()
      if not test_state.test_buf or not vim.api.nvim_buf_is_valid(test_state.test_buf) then
        return
      end
      if not test_state.term_buf or not vim.api.nvim_buf_is_valid(test_state.term_buf) then
        return
      end

      local output_lines = get_terminal_output(test_state.term_buf)
      local test_results = parse_pytest_output(output_lines)
      local test_functions = find_test_functions(test_state.test_buf)

      place_test_signs(test_state.test_buf, test_functions, test_results)
    end

    -- Find the test terminal buffer
    local function find_test_terminal()
    ...
    local function close_test_terminal()
      ...
      -- Clear state
      test_state.term_buf = nil
    end

    -- Check if pytest output indicates completion
    local function is_pytest_complete(lines)
    ...
    -- Poll terminal for test completion
    local function poll_for_completion()
      if not test_state.term_buf or not vim.api.nvim_buf_is_valid(test_state.term_buf) then
        return
      end

      local output_lines = get_terminal_output(test_state.term_buf)
      if is_pytest_complete(output_lines) then
        update_signs_from_terminal()
      else
        -- Keep polling every 500ms
        vim.defer_fn(poll_for_completion, 500)
      end
    end
    ...
    local function run_test(cmd)
      ...
      if current_file:match("test_") or current_file:match("_test%.py$") then
        test_state.test_file = current_file
        test_state.test_buf = current_buf
        -- Show running indicators
        place_running_signs(test_state.test_buf)
      end
      ...
      vim.defer_fn(function()
        test_state.term_buf = find_test_terminal()
        if test_state.term_buf then
          -- Start polling for test completion
          vim.defer_fn(poll_for_completion, 1000)
        end
      end, 300)
    end

    -- Clear signs keybinding
    local function clear_signs()
      if test_state.test_buf then
        clear_test_signs(test_state.test_buf)
      end
    end
    ...
    map("n", "<leader>tr", function()
      test_state.term_buf = find_test_terminal()
      test_state.test_buf = vim.api.nvim_win_get_buf(vim.api.nvim_get_current_win())
      if test_state.term_buf and test_state.test_buf then
        update_signs_from_terminal()
      end
    end, opts)
  end,
